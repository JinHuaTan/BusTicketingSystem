...
public class TopPlacesTableModel extends AbstractTableModel {
    ...
    public TopPlacesTableModel() throws SQLException {
    ...
    public void retrieveByDate(String date,String date2) throws SQLException {
        rs = stmt.executeQuery("SELECT D.DESTL AS \"Location\",COUNT(AB.DESTL) AS \"Total Ticket Sold\",SUM(AB.FAREPERTICKET) AS \"Total Sales (RM)\" FROM (SELECT D.DESTL ,T.TRANSID,TR.FAREPERTICKET FROM TRANS T,DSLOCATION D,ROUTE R,TRIP TR,TICKET TI "
            + "WHERE T.TRANSID=TI.TRANSID AND TI.TRIPID=TR.TRIPID AND TR.ROUTEID=R.ROUTEID AND R.DESTID=D.DESTID AND T.TRANSDATE >= '"+date+" 00:00:00' and T.TRANSDATE <= '"+date2+" 23:59:59') AB,DSLOCATION D WHERE D.DESTL=AB.DESTL GROUP BY D.DESTL ORDER BY SUM(AB.FAREPERTICKET) DESC");
    ...
    public static void main(String args[]) throws SQLException {
        ...
        frame.add(new JTable(new TopPlacesTableModel()));
